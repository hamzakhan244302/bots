...
def main_menu(level):
    kb = []
    if level >= 1:
        kb.append([InlineKeyboardButton("Status", callback_data="status"),
                   InlineKeyboardButton("Keys", callback_data="keys")])
        kb.append([InlineKeyboardButton("Users", callback_data="users"),
                   InlineKeyboardButton("Balance Info", callback_data="balance_menu")])
        kb.append([InlineKeyboardButton("Generate Key", callback_data="generate")])
    if level == 1:
        kb.append([InlineKeyboardButton("Set User Level", callback_data="set_level"),
                   InlineKeyboardButton("Add Balance", callback_data="add_balance")])
        kb.append([InlineKeyboardButton("Block User", callback_data="block_user"),
...
async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if user and user['level'] > 0:
        ...
        await update.message.reply_text(
            ...
            f"Send your Chat ID to the Owner: `{chat_id}`\n\n"
            "Owner will assign your level.",
            parse_mode='Markdown'
        )
